...
function PythonBuddyXBlock(runtime, element, params) {
$(document).ready(function(){
	...
	//Get unique id from pybuddy.py
	/*
	init: function(){
		data = params.data; 
	}
	*/
	//Check to see if I can get html_id
	console.log("sdf")
	console.log("hello worldsdfsdfsdfsd")
	console.log(params)
	console.log(element)
	console.log(runtime)
	console.log("test")
	//Array of editors
	/*
	OLD CODE WHICH ASSUMES THAT INSTRUCTOR WILL UPLOAD MULTIPLE PROBLEMS TO A PAGE
	var editors=[];
	var uid_list = [];
	function generateUID() {
	    ...
	    for (var i = 1; true; i++) {
	        ...
	        //Checks to see if uid is already created. If not, then it is created.
	        if ($.inArray(uid, uid_list)===-1){
	        	uid_list.push(uid);
	            return uid;
	        }
	    }
	...
	editor_uid = generateUID();
	console.log("editor_uid: "+editor_uid);
	*/
	//Add new editor element <= relies on getting unique html_id from python
	/*
	editors[editor_uid]=CodeMirror.fromTextArea(document.getElementById("txt"), {
        mode: {name: "python",
               version: 2,
    ...
    */
	...
	$( "#run").click(function(eventObject) {
        $.ajax({
            type: "POST",
            url: run_code,
